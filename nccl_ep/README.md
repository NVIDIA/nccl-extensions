...
(`NCCL_EP_LAYOUT_EXPERT_MAJOR`) in the default `kLocalPermute` mode. In that mode
the receiver expands the FLAT staging buffer into the user's recv buffer with a
**permute kernel** (SASS stores) and combine reads it back with a reduce kernel —
so the recv buffer is filled and read purely by kernel loads/stores. This matters
because `cudaMemcpy`/`cudaMemset` **crash** on a `cuMem` HOST_NUMA pointer (see
below), so only the kernel path safely drives an elastic buffer's CPU segment:
...
// GPU segment sized for the common case; CPU segment for the tail.
ncclEpElasticAlloc(&base, &buf, gpu_bytes, cpu_bytes, /*gpu_dev_id*/-1, /*numa_id*/-1);
