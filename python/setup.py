...
_NCCL_EP_SO = Path(__file__).parent / "nccl_extensions" / "ep" / "lib" / "libnccl_ep.so"
if not _NCCL_EP_SO.exists():
    print(
        f"WARNING: {_NCCL_EP_SO} not found. The built wheel will not "
        "include the NCCL EP shared library, and `import nccl_extensions.ep` will fail at "
...
PACKAGE = "nccl_extensions.bindings"
...
def libname_extensions(libname: str) -> list[Extension]:
    ...
    For libname="nccl_ep":
        nccl_extensions.bindings.nccl_ep              <- nccl_extensions/bindings/nccl_ep.pyx
        nccl_extensions.bindings.cynccl_ep             <- nccl_extensions/bindings/cynccl_ep.pyx
        nccl_extensions.bindings._internal.nccl_ep     <- nccl_extensions/bindings/_internal/nccl_ep_linux.pyx
    """
